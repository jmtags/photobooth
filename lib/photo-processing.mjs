...
export function buildPrompt(options) {
  ...
  return [
    ...
    "Only change the background and clothing when requested.",
    ...
    "If only the background is requested, preserve the whole person exactly and change only the background.",
    "If only clothing is requested, preserve the face, hair, skin, and body pose exactly and change only the clothing below the neck.",
    "If clothing and background edits are both requested, change only the background and the clothing below the neck.",
...
async function createEditMask(photoUrl, options) {
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      const insideHead =
        ((nx - 0.5) / 0.23) ** 2 + ((ny - 0.275) / 0.275) ** 2 <= 1 ||
        ((nx - 0.5) / 0.2) ** 2 + ((ny - 0.325) / 0.235) ** 2 <= 1;
      const insideNeck = ((nx - 0.5) / 0.11) ** 2 + ((ny - 0.515) / 0.075) ** 2 <= 1;
      const insideShoulders = ((nx - 0.5) / 0.35) ** 2 + ((ny - 0.68) / 0.18) ** 2 <= 1;
      const insideLowerTorso = ((nx - 0.5) / 0.3) ** 2 + ((ny - 0.88) / 0.2) ** 2 <= 1;
      const insidePerson = insideHead || insideNeck || insideShoulders || insideLowerTorso;
      const insideClothing =
        ny >= 0.46 &&
        ((((nx - 0.5) / 0.31) ** 2 + ((ny - 0.68) / 0.18) ** 2 <= 1) ||
          (((nx - 0.5) / 0.26) ** 2 + ((ny - 0.88) / 0.22) ** 2 <= 1));
      const editable = (needsBackgroundEdit && !insidePerson) || (needsAttireEdit && insideClothing);
...
export async function processPhoto({ photoUrl, options }) {
  ...
  const editMask = await createEditMask(photoUrl, effectiveOptions);
