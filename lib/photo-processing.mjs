...
async function isLikelyBlankResult(photoUrl, background) {
  const { data, info } = await sharp(dataUrlToBuffer(photoUrl))
    .ensureAlpha()
  ...
  const targetBackground =
    background === "white" ? [255, 255, 255] : background === "blue" ? [37, 99, 235] : null;

  let personPixels = 0;
  let visibleSubjectPixels = 0;

  for (let y = 0; y < info.height; y += 1) {
    for (let x = 0; x < info.width; x += 1) {
      const nx = (x + 0.5) / info.width;
      const ny = (y + 0.5) / info.height;
      if (!getPortraitRegions(nx, ny).insidePerson) {
        continue;
      }
      ...
      const distance = Math.abs(r - targetBackground[0]) + Math.abs(g - targetBackground[1]) + Math.abs(b - targetBackground[2]);
      if (distance > 42) {
        visibleSubjectPixels += 1;
      }
    }
  ...
  return visibleSubjectPixels / personPixels < 0.08;
}
...
export async function processPhoto({ photoUrl, options }) {
  ...
  if (needsBackgroundEdit) {
    try {
      ...
      const backgroundResult = await runOpenAIEdit({
      ...
      if (await isLikelyBlankResult(backgroundResult, effectiveOptions.background)) {
        return {
          processedPhotoUrl: photoUrl,
          notice:
            "The processed image came back empty, so we kept your original photo instead.",
        };
      }

      workingPhotoUrl = backgroundResult;
    } catch (error) {
  ...
  try {
    ...
    const compositedPhotoUrl = await compositeEditedClothing({
      basePhotoUrl: workingPhotoUrl,
      attireCrop,
      editedCropUrl: editedAttireCropUrl,
    });

    if (await isLikelyBlankResult(compositedPhotoUrl, effectiveOptions.background)) {
      return {
        processedPhotoUrl: workingPhotoUrl,
        notice:
          "The clothes result came back empty, so we kept the safer original version.",
      };
    }

    return {
      processedPhotoUrl: compositedPhotoUrl,
      notice: null,
    };
  } catch (error) {
    if (isSafetyRejection(error)) {
