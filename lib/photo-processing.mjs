...
async function restoreOriginalHead({ originalPhotoUrl, editedPhotoDataUrl }) {
  ...
  const maskSvg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
      ...
      <ellipse cx="${width * 0.5}" cy="${height * 0.31}" rx="${width * 0.24}" ry="${height * 0.30}" fill="white"/>
      <ellipse cx="${width * 0.5}" cy="${height * 0.58}" rx="${width * 0.14}" ry="${height * 0.10}" fill="white"/>
      <ellipse cx="${width * 0.5}" cy="${height * 0.68}" rx="${width * 0.24}" ry="${height * 0.08}" fill="white"/>
    </svg>
  `;
