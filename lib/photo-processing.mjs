...
export function buildPrompt(options) {
  const instructions = [
    "Keep the exact original person, pose, framing, face, head shape, hair, and facial expression unchanged.",
    "Do not change any part of the head or face.",
  ];

  if (options.background === "white") {
    instructions.push("Replace the background with a clean plain white background only.");
  }

  if (options.background === "blue") {
    instructions.push("Replace the background with a clean plain blue ID-photo background only.");
  }

  if (options.background === "removed") {
    instructions.push("Remove the background only and return a transparent PNG cutout.");
  }

  if (options.attire === "male-office") {
    instructions.push("Change the clothing to formal office attire only: dark blazer and light collared shirt.");
  }

  if (options.attire === "female-office") {
    instructions.push("Change the clothing to formal office attire only: professional blazer and modest blouse.");
  }

  instructions.push("Do not add accessories, filters, text, logos, or any extra objects.");
