...
async function restoreOriginalHead({ originalPhotoUrl, editedPhotoDataUrl }) {
  ...
  const maskSvg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
      ...
      <ellipse cx="${width * 0.5}" cy="${height * 0.285}" rx="${width * 0.235}" ry="${height * 0.285}" fill="white"/>
      <ellipse cx="${width * 0.5}" cy="${height * 0.515}" rx="${width * 0.115}" ry="${height * 0.075}" fill="white"/>
      <ellipse cx="${width * 0.5}" cy="${height * 0.595}" rx="${width * 0.18}" ry="${height * 0.05}" fill="white"/>
    </svg>
  `;
