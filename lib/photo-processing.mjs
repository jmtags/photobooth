...
export function buildPrompt(options) {
  ...
  return [
    ...
    "Do not change any part of the head.",
    "Preserve the original face, hair, hairline, ears, eyebrows, eyes, nose, lips, jawline, skin texture, expression, gaze, and head shape exactly as captured.",
    "Do not beautify, reshape, smooth, de-age, restyle, relight, or reinterpret the head or face.",
    "Do not change the hairstyle, hair color, or any facial expression.",
    "The protected head region from the input must remain unchanged.",
...
function getImageSize(buffer) {
  ...
  throw new Error("Unsupported image format for head-protection mask.");
}

function createHeadProtectionMask(width, height) {
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      const headEllipse =
        ((nx - 0.5) / 0.23) ** 2 + ((ny - 0.31) / 0.29) ** 2 <= 1;
      const neckBridge =
        ((nx - 0.5) / 0.11) ** 2 + ((ny - 0.58) / 0.08) ** 2 <= 1;
      const protectHead = headEllipse || neckBridge;
      const idx = (width * y + x) << 2;

      png.data[idx] = 0;
      png.data[idx + 1] = 0;
      png.data[idx + 2] = 0;
      png.data[idx + 3] = protectHead ? 255 : 0;
    }
  }
...
export async function processPhoto({ photoUrl, options }) {
  ...
  const { width, height } = getImageSize(dataUrlToBuffer(photoUrl));
  const headProtectionMask = createHeadProtectionMask(width, height);
