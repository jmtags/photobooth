...
async function createHeadProtectionMask(photoUrl) {
  ...
  const maskSvg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
      ...
      <ellipse cx="${width * 0.5}" cy="${height * 0.29}" rx="${width * 0.24}" ry="${height * 0.30}" fill="white"/>
      <ellipse cx="${width * 0.5}" cy="${height * 0.56}" rx="${width * 0.12}" ry="${height * 0.09}" fill="white"/>
      <ellipse cx="${width * 0.5}" cy="${height * 0.64}" rx="${width * 0.20}" ry="${height * 0.06}" fill="white"/>
    </svg>
  `;
